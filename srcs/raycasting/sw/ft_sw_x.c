/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_sw_x.c                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: orfreoua <ofreoua42student@gmail.com>      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/18 22:07:46 by orfreoua          #+#    #+#             */
/*   Updated: 2023/03/22 17:51:23 by orfreoua         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../headers/cub3d.h"

double	*ft_sw_x(t_data *data, t_point decimal, double angle)
{
	int		i;
	double	*hypo;

	i = 1;
	hypo = malloc(sizeof(double) * 2);
	if (hypo == NULL)
		return (NULL);
	hypo[0] = decimal.x / cos((M_PI / 2) - (angle - (M_PI / 2)));
	data->args.player.hit1.x = (int)data->args.player.pos.x;
	data->args.player.hit1.y = data->args.player.pos.y + sqrt(hypo[0] * hypo[0] - (decimal.x * decimal.x));
	while (data->args.player.hit1.y <= data->args.map.size.height && data->args.player.hit1.x >= 0
		&& ft_check_hit(data, 'w', data->args.player.hit1.x,
			data->args.player.hit1.y) != '1')
	{
		hypo[0] = (decimal.x + i) / cos((M_PI / 2) - (angle - (M_PI / 2)));
		data->args.player.hit1.x = (int)data->args.player.pos.x - i;
		data->args.player.hit1.y = data->args.player.pos.y + sqrt(
				hypo[0] * hypo[0] - ((decimal.x + i) * (decimal.x + i)));
		i++;
	}
	hypo[1] = ft_sw_x_1(data, decimal, angle);
	return (hypo);
}

double	ft_sw_x_1(t_data *data, t_point decimal, double angle)
{
	int		i;
	double	hypo;

	i = 1;
	hypo = (1 - decimal.y) / cos(angle - (M_PI / 2));
	data->args.player.hit2.x = data->args.player.pos.x - sqrt(
			hypo * hypo - ((1 - decimal.y) * (1 - decimal.y)));
	data->args.player.hit2.y = (int)data->args.player.pos.y + 1;
	while (data->args.player.hit2.y <= data->args.map.size.height && data->args.player.hit2.x
		>= 0 && ft_check_hit(data, 's',
			data->args.player.hit2.x, data->args.player.hit2.y) != '1')
	{
		hypo = ((1 - decimal.y) + i) / cos(angle - (M_PI / 2));
		data->args.player.hit2.x = (int)data->args.player.pos.x + decimal.x - sqrt(
				hypo * hypo - (((1 - decimal.y) + i) * ((1 - decimal.y) + i)));
		data->args.player.hit2.y = (int)data->args.player.pos.y + 1 + i;
		i++;
	}
	return (hypo);
}
