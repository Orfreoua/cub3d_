...
/*   Updated: 2023/03/22 17:55:46 by orfreoua         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	*ft_ne_y(t_data *data, t_point decimal, double angle)
{
	...
	while (data->args.player.hit1.y >= 0 && data->args.player.hit1.x <= data->args.map.size.width
		&& ft_check_hit(data, 'n', data->args.player.hit1.x,
	...
	hypo[1] = ft_ne_y_1(data, decimal, angle);
	return (hypo);
}
...
double	ft_ne_y_1(t_data *data, t_point decimal, double angle)
{
	...
	while (data->args.player.hit2.y >= 0 && data->args.player.hit2.x <= data->args.map.size.width
		&& ft_check_hit(data, 'e', data->args.player.hit2.x,
